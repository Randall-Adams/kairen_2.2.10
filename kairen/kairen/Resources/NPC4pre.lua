...
NPCs = setmetatable(
{
    ...
    Spawn = (function(_nameSafe)
        if NPCs.Find("nameSafe", _nameSafe) ~= nil then
            ...
            elseif FAPI.IO.File_Exists(RF.EQOA.Game_Data.NPC_Maker.self .. _nameSafe .. Extension_NPCFiles) == true then
                _filepath = RF.EQOA.Game_Data.NPC_Maker.self .. _nameSafe .. Extension_NPCFiles
                print(_nameSafe .. " located in " .. RF.EQOA.Game_Data.NPC_Maker.self .. ".")
            else
    ...
    UpdateVisualNPCs = (function()
        if Ghosts.NumberofGhosts == 0 then 
            ...
            print("Ghosts.NumberofGhosts = " .. Ghosts.NumberofGhosts)
        end
        ...
        if _nameSafe == nil then
            ...
            if FAPI.IO.File_Exists(fileOfGhost) == false then
                ...
                if fileversion == "0.1.1" then
                    ...
                    self.X = FAPI.CE.Address.New(FAPI.IO.ReadNextLine(file, "%-%-"), "Float", "Ghost[" .. self.nameSafe .. "].X");
                    self.Y = FAPI.CE.Address.New(FAPI.IO.ReadNextLine(file, "%-%-"), "Float", "Ghost[" .. self.nameSafe .. "].Y");
                    self.Z = FAPI.CE.Address.New(FAPI.IO.ReadNextLine(file, "%-%-"), "Float", "Ghost[" .. self.nameSafe .. "].Z");
                    self.F = FAPI.CE.Address.New(FAPI.IO.ReadNextLine(file, "%-%-"), "Float", "Ghost[" .. self.nameSafe .. "].F");
                    self.Level = FAPI.CE.Address.New(FAPI.IO.ReadNextLine(file, "%-%-"), "Bytes", "Ghost[" .. self.nameSafe .. "].Level", 1);
                    self.HP = FAPI.CE.Address.New(FAPI.IO.ReadNextLine(file, "%-%-"), "Bytes", "Ghost[" .. self.nameSafe .. "].HP", 1);
                    function self.Name(_value)
                        if _value ~= nil then
