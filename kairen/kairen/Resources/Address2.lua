...
Code-Version: 1.0
Code-Description: Represents a CE Address through a LUA Class.
...
function(_Address0x, _Type, _Name, _Length, _SubType)
    ...
    if self.vType == "Bytes" then
        ...
        function self.Value(_new)
            if _new == nil then
                return readBytes(self.vAddress0x, vLength)
            else
    ...
    elseif self.vType == "String" then
        ...
        elseif self.vSubType == "Wide" then
            function self.Value(_new)
                if _new == nil then
                    return readString(self.vAddress0x, vLength, true)
                else
                    writeString(self.vAddress0x, _new)
                end
            end
        elseif self.vSubType == "Type1" then
            function self.Value(_new)
                if _new == nil then
        ...
        elseif self.vSubType == "Type2" then
            function self.Value(_new)
                if _new == nil then
