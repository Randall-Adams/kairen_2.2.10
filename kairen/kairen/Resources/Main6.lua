...
sleep(333)
keyUp(VK_F3)
if SilentMode == nil then
    print("SCRIPT STARTED")
elseif SilentMode == false then
    print("SCRIPT STARTED")
end
local thestring = debug.getinfo(1).source:match("@(.*)$")
local ts2 = string.gsub(thestring , "Main","ConsoleOutput")
local ts2len = string.len(thestring ) + 9
local ts2 = string.sub(ts2, 1, ts2len)
local ts3 = string.gsub(thestring , "Main","File Paths")
local ts3len = string.len(thestring ) + 6
local ts3 = string.sub(ts3, 1, ts3len)
--print(string.gsub(debug.getinfo(1).source:match("@(.*)$"), "Main","File Paths"))
dofile(ts2)
dofile(ts3)
--dofile(RF.EQOA.LUAs.self .. "ConsoleOutput.lua")
...
dofile(RF.EQOA.LUAs.Modules.FAPI.self .. "Classes/NPC.lua")
dofile(RF.EQOA.LUAs.Modules.FAPI.self .. "Classes/SpawnHandler.lua")
...
--stringlist_add(aalist,"pcsx2-r3878.exe");
...
GameLoop.LogicLoop = {};
GameLoop.LogicLoop.ClearOldFiles = (function ()
...
GameLoop.LogicLoop.RunSettingsFile = (function ()
            if _G.mode == "" then
                print("Error: No Mode Set -- Setting Console Mode")
            ...
            if FAPI.IO.File_Exists(RF.EQOA.LUAs.Modes.self .. mode .. "/" .. mode .. "-Settings" .. ".lua") then 
                ...
                print("Error: No Settings File Found -- Using Console Mode Settings")
                _G.mode = "Console Mode"
                GameLoop.et.check("SETUP", mode, 200)
            end
        end);
...
GameLoop.LogicLoop.LoopFunction = (function ()
                ...
                if FAPI.IO.File_Exists(GameLoop.LogicLoop.GameLogicLoopFile) == true then
                    ...
                    print("Error: No Mode File Found")
                end
