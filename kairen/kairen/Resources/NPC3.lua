...
_G.NPCs = setmetatable(
{
    New = (function(_nameSafe)
        local _filePath = RF.EQOA.Custom_Data.NPC_Maker.self .. _nameSafe .. Extension_NPCFiles
        if _G.FAPI.IO.File_Exists(_filePath) == false then
            print(_nameSafe .. " File not found.")
            return nil;
        end
        local i = 1
        _file = FAPI.IO.Read_FileToStringArray(_filePath, FAPI.CommentIndicatorString)
        if _file[i] == "0.1.1" then
            print("NPC is 0.1.1")
            print("NPC file needs converted to 0.2.0 before it can spawn.")
            return nil;
        elseif _file[i] == "0.1.2" then
            print("NPC is 0.1.2")
            print("NPC file needs converted to 0.2.0 before it can spawn.")
            return nil;
        elseif _file[i] == "0.1.3" then
            print("NPC is 0.1.3")
            print("NPC file needs converted to 0.2.0 before it can spawn.")
            return nil;
        elseif _file[i] == "0.2.0" then
            print("NPC is 0.2.0")
        else
        ...
        function self.Zone()
            local _rowcolumn
            _columnrow = EQOA.HelperFunctions.GetZoneRowColumn_ByCoords(self.X(), self.Y())
        ...
        self.FileVersion_Loaded = "0.2.0"
        i = i + 1
        self.nameSafe = _file[i]
        self.isSpawned = true;
        i = i + 1
        self.Name(_file[i])
        i = i + 1
        ...
        i = i + 1
        self.F(_file[i])
        i = i + 1--_file[i] --        NPCs[i].Race = _file[i]
        i = i + 1--_file[i] --        NPCs[i].Gender = _file[i]
        i = i + 1 --_file[i] --        NPCs[i].Class = _file[i]
        i = i + 1
        self.level(_file[i])
        i = i + 1
        self.hp(_file[i])
        --_file[i]--        NPCs[i].MP = _file[i]
        --_file[i]--        NPCs[i].AC = _file[i]
        --_file[i]--        NPCs[i].Strength = _file[i]
        --_file[i]--        NPCs[i].Stamina = _file[i]
        --_file[i]--        NPCs[i].Agility = _file[i]
        --_file[i]--        NPCs[i].Dexterity = _file[i]
        --_file[i]--        NPCs[i].Wisdom = _file[i]
        --_file[i]--        NPCs[i].Intelligence = _file[i]
        --_file[i]--        NPCs[i].Charisma = _file[i]
        --_file[i]--        NPCs[i].FR = _file[i]
        --_file[i]--        NPCs[i].CR = _file[i]
        --_file[i]--        NPCs[i].LR = _file[i]
        --_file[i]--        NPCs[i].AR = _file[i]
        --_file[i]--        NPCs[i].PR = _file[i]
        --_file[i]--        NPCs[i].DR = _file[i]
        return self;
    end
    );
    New2 = (function(_nameSafe)
        ...
        function self.ApplyFileData()
          --print("_G.NPCs.New.ApplyFileData()")
            ...
            --co(_sender, "co_debugoutput", "File Version is: " .. FileVersion)
                ...
                elseif FileVersion == "0.1.2" then
                  --print("NPC Version Matched")
                    self.FileVersion_Loaded = "0.1.2"
                    self.nameSafe = _G.FAPI.IO.ReadNextLine(npcfile, "%-%-")
                    self.isSpawned = true;
                    self.Name(_G.FAPI.IO.ReadNextLine(npcfile, "%-%-"))
                    self.X(_G.FAPI.IO.ReadNextLine(npcfile, "%-%-"))
    ...
    AssignNextNPC = (function(_npcToAssign)
      ...
      --print("type(_npcToAssign) == " .. type(_npcToAssign))
        ...
        _G.Kanizah.UpdateItemByAdditionalData("NPC" .. _G.NPCs.index .. "Name", _G.NPCs[_G.NPCs.index].Name)
        _G.Kanizah.UpdateItemByAdditionalData("NPC" .. _G.NPCs.index .. "X", _G.NPCs[_G.NPCs.index].X)
